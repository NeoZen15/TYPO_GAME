...
`npm run quality` enchaîne : `lint`, `typecheck`, `check:artifacts`, `check:compat-bridges`, `check:dev-routes`, `check:runtime-boundaries`, `check:copy`, `check:contracts`, `check:typography-contract`, puis `build`.

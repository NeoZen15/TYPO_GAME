...
État par sujet : **17 faits · 0 en cours · 16 à faire · 2 bloqueurs · 6 parkés / à décider** (41 sujets).
...
- [x] **Livrer le fichier de licence avec chaque police auto-hébergée** · `Fait` (2026-07-28)
  ...
  _**Reste ouvert.** 1. Les 5 enregistrements sans `license_url` sont les 5 polices système `arial`, `courier_new`, `georgia`, `helvetica`, `times_new_roman`, toutes `proprietary`, toutes désactivées et **sans aucun dossier dans `public/fonts`**, donc rien n'est hébergé et l'obligation ne les concerne pas ; elles restent à remplacer par du libre avant lancement, décision produit déjà notée en section F. 2. La licence webfont de PP Frama, bloqueur inchangé. 3. `scripts/mirror_fonts.py`, qui crée les dossiers de `public/fonts`, ne pose pas encore la licence lui même : une nouvelle vague de polices arriverait sans licence, et c'est la porte qui l'attraperait, à corriger en appelant le script de synchronisation dans le pipeline. 4. Les 5 Ubuntu resteront en `license_type: unknown` jusqu'à la migration 010, non appliquée._
  - `public/fonts/`, `scripts/sync-font-licenses.mjs`, `scripts/quality/check-font-licenses.mjs`, `package.json`, `content/catalog/typefaces-core.json`

...
Il ne remplace pas `docs/game/training-engine-spec-v2-clean.md` : cette spec reste la source de vérité du **fonctionnement** du moteur (invariants I-01 à I-14, fenêtres d'intervalle, sélection, cooldowns). Le présent document ajoute les invariants **I-15 à I-27** et tranche ce qui restait ambigu.
...
| I-26 | **Le parcours personnel est premier et autonome** (décision du propriétaire, 2026-09-10). Un utilisateur peut ouvrir DWIGGINS quand il veut, jouer en entraînement, progresser dans son pool, découvrir de nouvelles typographies et recevoir ses propres recommandations, **sans école, sans classe, sans professeur et sans assignation**. Aucune fonction du produit ne peut exiger une assignation pour jouer, et l'espace professeur est un **second parcours qui coexiste** avec le premier, jamais un préalable et jamais un remplaçant. Vérifiable par revue : un compte sans appartenance de classe accède à l'entraînement complet et à ses recommandations. |
| I-27 | **Les recommandations existent des deux côtés, et jamais sur les données de l'autre.** Le professeur reçoit des recommandations d'exercices pour sa classe, calculées **uniquement** sur les assignations qu'il a données. L'élève reçoit ses propres recommandations, calculées **uniquement** sur son état personnel. Les deux partagent leur méthode et jamais leurs sources : une recommandation d'élève ne lit aucune donnée institutionnelle, une recommandation de professeur ne lit aucune donnée personnelle (I-15, I-16). |
...
**F. Deux parcours qui coexistent : VERROUILLÉ le 2026-09-10.** Le risque nommé par le propriétaire, et il était réel : en construisant l'espace professeur, DWIGGINS pouvait glisser vers un produit scolaire où l'élève ne joue que quand un enseignant lui donne quelque chose. Ce n'est pas le projet. Le parcours personnel historique reste entièrement vivant et **se suffit à lui même** ; l'assignation est un second parcours. Les quatre contextes moteur qui en découlent, leurs droits de lecture et d'écriture sur l'état personnel, et le sens unique de circulation entre les deux mondes sont formalisés en matrice dans `architecture-backend.md` §2.2. Inscrit en **I-26**, et la symétrie des recommandations en **I-27**.

...
// ---------------------- 4b. self-test of the day-key arithmetic (daily progression)
//
// The daily progression (streak, activity window, record) used to be counted in
// SESSIONS, which double-counts the moment two sessions can be created from one
// page load. It must be counted in EVENTS instead, on calendar days already
// resolved to Europe/Paris by SQL. This self-tests the pure arithmetic on
// synthetic day keys, no database involved.
...
console.log(
  "check:session-lifecycle OK : no round cap, answering never closes a session, " +
    ...
    "bilan maths verified on 5 synthetic rows, day-key arithmetic verified on synthetic keys."
);
